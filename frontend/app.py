    ...
    if "result" not in st.session_state:
        ...
        with tab3:
            ...
            for i, d in enumerate(damages):
                ...
                with st.expander(f"🔧 Damage {i+1} - {d['part']}"):
                    st.markdown(f"<p><b>Type:</b> {d['damage_type']}</p>", unsafe_allow_html=True)
                    st.markdown(f"<p class='{severity}'><b>Severity:</b> {severity.upper()}</p>", unsafe_allow_html=True)
                    st.write(f"Confidence: {d['confidence']}%")
                    st.write(f"Area: {d['area']}")
                    st.write(f"Damage Ratio: {d['damage_ratio']}")
