...
#Added test mode to allow bypassing strict checks during testing

from datetime import datetime

def check_fraud(exif, incident_date, test_mode=True):
    """
    Fraud detection with test mode support
    """

    # 🧪 TEST MODE (skip strict checks)
    if test_mode:
        if not exif:
            return False, "No EXIF (allowed in test mode)"

        if "DateTimeOriginal" not in exif:
            return False, "No timestamp (allowed in test mode)"
    ...
    try:
        if "DateTimeOriginal" in exif:
            ...
            diff = (incident_date_obj.date() - photo_date.date()).days

            if diff > 2:
                return True, "Image captured well before incident"

            if diff < 0:
                return True, "Image captured after incident"

    except Exception as e:
        return True, f"Metadata parsing error: {str(e)}"

    return False, "Valid (or test mode bypass)"