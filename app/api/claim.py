...
#     vehicle_result = {
#     "make": schedule_data.get("make", "Unknown"),
#     "model": schedule_data.get("model", "Unknown"),
#     "year": 2021,
#     "confidence": 100,
#     "source": "policy"
#      }

#     print("🚗 VEHICLE FROM POLICY:", vehicle_result)

#     damage_result = detect_damage(saved_paths)
#     damages = damage_result["damages"]

#     # vehicle_age = 2026 - int(vehicle_result.get("year", 2021))
#     vehicle_year = vehicle_result.get("year", 2021)
...
async def process_claim(
    ...
    # -------------------------------
    # STEP 1 — SAVE IMAGES + FRAUD CHECK
    # -------------------------------
    for file in files:
        contents = await file.read()
        file_path = os.path.join(UPLOAD_FOLDER, file.filename)
    ...
    # -------------------------------
    # STEP 2 — SAVE POLICY PDF
    # -------------------------------
    schedule_path = os.path.join(POLICY_FOLDER, policy_schedule.filename)

    contents = await policy_schedule.read()
    with open(schedule_path, "wb") as f:
        f.write(contents)
    ...
    print("📑 POLICY DATA:", schedule_data)

    # -------------------------------
    # STEP 4 — VEHICLE FROM POLICY
    # -------------------------------
    vehicle_result = {
        "make": schedule_data.get("make", "Unknown"),
        "model": schedule_data.get("model", "Unknown"),
        "year": schedule_data.get("year", 2021),
        "confidence": 100,
        "source": "policy"
    }
    ...
    # -------------------------------
    damage_result = detect_damage(saved_paths)
    damages = damage_result.get("damages", [])

    print("🔧 DAMAGE RESULT:", damage_result)

    # -------------------------------
    # STEP 7 — IDV CALCULATION
    # -------------------------------
    idv_result = calculate_idv(
        vehicle_result["make"],
    ...
    # Prefer policy IDV if available
    idv_value = schedule_data.get("idv", 0) or idv_result["idv"]
    ...
    print("🛠️ REPAIR RESULT:", repair_result)

    # -------------------------------
    # STEP 9 — POLICY COVERAGE (RAG)
    # -------------------------------
    policy_wording_path = "data/policies/policy_wording_bajaj.pdf"
    ...
    pdf_path = generate_pdf_report(
        claim_id=claim_id,
        vehicle_make=vehicle_result["make"],
        vehicle_model=vehicle_result["model"],
